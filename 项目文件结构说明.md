...
- `background/steps/wait-registration-success.js`：步骤 6 实现，负责注册资料提交后等待 20 秒，让注册成功状态和页面跳转稳定；默认开启侧栏第六步 `清 Cookies` 开关，在等待结束后清理 ChatGPT / OpenAI 相关 cookies，用户也可以手动关闭。
